...
const DayWiseReport = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h1 className="text-2xl font-bold text-gray-900 mb-6">Day Wise Report (Business Summary)</h1>
                        ...
                        {searched && reportData.length === 0 ? (
                            ...
                            reportData.map((record, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{record.company_name}</td>
            ...
            {reportData.length > 0 && (
                <div className="mt-4 text-right font-bold text-xl">
                    Total Revenue: {reportData.reduce((acc, curr) => acc + (parseFloat(curr.total_amt) || 0), 0).toFixed(2)}
                </div>
            )}
        </div>
    );
